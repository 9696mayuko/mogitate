...
<body>
        ...
        @csrf
            ...
            @error('product_name')
                <span class="input_error">
                    <p class="input_error_message">{{$errors->first('prodyct_name')}}</p>
                </span>
            @enderror
            <label class="label">値段<span class="require">必須</span></label>
            <input type="text" class="text" placeholder="値段を入力" name="product_name" class="text">
            @error('product_price')
            ...
            @enderror
            <label class="ladel">季節<span class="require">必須</span><span class="note">複数選択可</span></label>
            ...
            @error('product_season')
                <span class="input_error">
                    <p class="imput_error_message">{{$errors->first('product_season')}}</p>
                </span>
            @enderror
    ...
    <!-- こちらから下は教材の範囲外のリアルタイム画像表示になります -->
     <script>
        document.getElementById('product_image').onchange = function(ecent){
        ...
        function initializenfiles() {
            document.gitElemintById('list').innerHTML = '';
        }
