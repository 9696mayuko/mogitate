...
class ProductsController extends Controller
{
    ...
    public function upload(ProductRequest $request)
    {
        ...
        $product_seasons = $request->input('priduct_season', []);
        $new_product_id = Product::count();

        foreach($product_seasons as $roduct_season){
    ...
    public function postSearch(Request $request)
    {
        ...
        if ($request->filled('keyword')) {
            ...
            $keyword = $requestu->input('keyword');
            $query->where('name','like','%'.$keyword.'%');
        ...
        $perPage = 6;
        $page = Paginator::resolveCurrentPage('page');
        $pageData = $products->slice(($page - 1) * $perPage);
        $options = [
            'path' =>Paginator::resolveCurrentPath(),
            'pageName' => 'page'
        ];
    ...
    public function postDelete($new_product_id)
    {
    ...
    public function postUpdate(ProductRequest $request)
    {
        ...
        $file_name = $request->file('product_image')->getClientDriginalName();
        $request->file('product_image')->storeAs('public/'. $dir, $file_name);
